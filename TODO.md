...
- [x] Add documentation for `.env` and database setup
- [x] Create project-level `CLAUDE.md` in repo root
- [x] Create global `~/.claude/CLAUDE.md` for personal defaults
- [ ] Add automated tests for ingestion and embedding logic

## Medium priority

- [ ] Add support for Notion exports or Notion API ingestion
- [ ] Add text chunking to ingest_data.py (all-MiniLM-L6-v2 truncates at 256 tokens; chunk at ~200 tokens with overlap)
- [ ] Add a reusable data schema for documents and metadata
- [ ] Improve metadata capture (source, author, origin path, content type)
- [x] Add better error handling and retry logging across download/ingest steps
...
- [x] Add incremental sync so repeated runs only ingest new or changed files
